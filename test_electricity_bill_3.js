...
async function testFetchElectricityBill() {
    ...
    try {
        ...
        const operatorCode = 54; // APEPDCL
        
        const paramRes = await axios.get(`${EKO_BASE_URL}/customer/payment/bbps/operator/${operatorCode}/parameters?initiator_id=${EKO_INITIATOR_ID}`, { headers });
        const params = paramRes.data.param_attributes?.list_elements || [];
        console.log("Params:", params.map(p => p.param_name));
        ...
        // Add dummy values for all params
        params.forEach(p => {
            queryParams.append(p.param_name, "1234567890123"); 
        });
